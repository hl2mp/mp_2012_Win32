...
bool CHL2MP_Player::BumpWeapon( CBaseCombatWeapon *pWeapon )
{
	...
	if ( bOwnsWeaponAlready == true ) 
	{
		//If we have room for the ammo, then "take" the weapon too.
		 if ( Weapon_EquipAmmoOnly( pWeapon ) )
		 {
			...
			if( weaponstay.GetInt() == 0 || FClassnameIs( pWeapon, "weapon_medkit") || FClassnameIs( pWeapon, "weapon_frag") || FClassnameIs( pWeapon, "weapon_slam") || secondary <= 0 && primary <= 0 ) {
	...
	if( !IsSuitEquipped() && !FClassnameIs( pWeapon, "weapon_medkit") ) {
		EquipSuit( false );
	}
