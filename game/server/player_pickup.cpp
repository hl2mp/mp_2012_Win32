...
void Pickup_ForcePlayerToDropThisObject( CBaseEntity *pTarget )
{
	...
	if (pPlayer && pPhysics->GetGameFlags() & FVPHYSICS_PLAYER_HELD )
	{
...
bool Pickup_OnAttemptPhysGunPickup( CBaseEntity *pPickedUpObject, CBasePlayer *pPlayer, PhysGunPickup_t reason )
{
 ...
 //               return false;
	...
	if( !IsAdmin( pPlayer, FL_ADMIN_PICKUP_COMBINE_BALL ) && FClassnameIs(pPickedUpObject,"prop_combine_ball") ) {
		CBaseEntity *pOwner = pPickedUpObject->GetOwnerEntity();
		if( pOwner != NULL )
			return false;
	}

	if( !IsAdmin( pPlayer, FL_ADMIN_PICKUP_NPC ) && pPickedUpObject->IsNPC() ) {
	...
	if( !IsAdmin( pPlayer, FL_ADMIN_PICKUP_VEHICLE ) && veh ) {
