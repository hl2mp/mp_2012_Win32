			...
			if ( pBCC && pVictimBCC )
			{
				...
				if ( m_bDamageAnyNPC || pBCC->IRelationType( pEntity ) == D_HT || pEntity->IsPlayer() )
				{
					if ( info.GetDamage() )
					{
						...
						if ( !pEntity->IsPlayer() || GlobalEntity_GetState( "gordon_precriminal_off" ) == GLOBAL_OFF )
						{
...
Vector CNPC_MetroPolice::GetActualShootTrajectory( const Vector &shootOrigin )
{
	...
	case BURST_LOCK_ON_AFTER_HIT:
		...
		if (  GetShootTarget() && GetShootTarget()->CollisionProp()->IsPointInBounds( m_vecBurstTargetPos ) )
		{
