...
void CNPC_Portal_FloorTurret::Shoot( const Vector &vecSrc, const Vector &vecDirToEnemy, bool bStrict )
{
	...
	//hl2mp.ru
	if( ClassMatches("npc_portal_turret_rpg") && pEnemy && FindInList( s_AllowNPC, pEnemy->GetClassname() ) ) {
		if( !m_hMissile && m_fNextMissile < gpGlobals->curtime) {
			QAngle vecAngles;
			...
			m_fNextMissile = gpGlobals->curtime + RandomFloat(1.0, 2.0);
		}
	}
