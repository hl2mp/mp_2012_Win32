...
extern char st_szOldMap[32];
extern char st_szNextSpot[32];
extern char st_szNextMap[32];

extern void LoadAdjacentEnts(const char* pOldLevel, const char* pLandmarkName);
extern int LoadGameState(char const* level, bool createPlayers);
extern void ResetSaveData(void);
...
bool CServerGameDLL::LevelInit( const char *pMapName, char const *pMapEntities, char const *pOldLevel, char const *pLandmarkName, bool loadGame, bool background )
{
	...
	if (st_szOldMap[0] != '\0' && (strcmp(st_szNextMap, pMapName) == 0)) {
		pOldLevel = st_szOldMap;
		pLandmarkName = st_szNextSpot;
		loadGame = true;
		st_szNextMap[0] = '\0';
	}
	else {
	...
	CUtlBuffer buf;
	filesystem->ReadFile(UTIL_VarArgs("maps/ent_cache/%s.custom", pMapName), "MOD", buf);
	if (!filesystem->ReadFile(UTIL_VarArgs("maps/ent_cache/%s.ent", pMapName), "MOD", buf))
		buf.PutString(pMapEntities);
	...
	if ( loadGame )
	{
		...
		if ( pOldLevel )
		{
			LoadAdjacentEnts( pOldLevel, pLandmarkName );
		}
...
extern bool SaveGameState( bool bTransition, CSaveRestoreData **ppReturnSaveData );

// Called when a level is shutdown (including changing levels)
void CServerGameDLL::LevelShutdown( void )
{
	if( st_szOldMap[0] != '\0' )
		SaveGameState( true, NULL );
	...
	//hl2mp.ru Не нашел я исходный код нужный!
...
void CServerGameClients::ClientDisconnect( edict_t *pEdict )
{
	...
	if ( player )
	{
		//hl2mp.ru - Client SaveRestore
		SaveClientState( player );
