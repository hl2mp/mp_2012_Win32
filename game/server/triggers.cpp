...
bool CBaseTrigger::PassesTriggerFilters(CBaseEntity *pOther)
{
	...
	if( !FClassnameIs( this, "trigger_push") && !FClassnameIs( this, "trigger_hurt") && (pOwner && FClassnameIs(pOwner,"info_npc_random")) ||
		FClassnameIs(pOther,"npc_merchant") )
		return false;

	if( pOther->IsNPC() && pOther->GetPlayerMP() ) {
		if( FClassnameIs( this, "trigger_teleport") )
...
void CChangeLevel::Spawn( void )
{
	...
	{
		CBaseEntity *pSnow = Create( "env_embers", GetAbsOrigin(), GetAbsAngles() );
		if( pSnow ) {
			pSnow->KeyValue("density","500");
			pSnow->KeyValue("lifetime","2");
			pSnow->KeyValue("rendercolor", "255 255 255");
			pSnow->KeyValue("model", STRING( GetModelName() ) );
			DispatchSpawn( pSnow );
		}
		pSnow = NULL;
		pSnow = Create( "env_embers", GetAbsOrigin(), GetAbsAngles() );
		if( pSnow ) {
			pSnow->KeyValue("density","500");
			pSnow->KeyValue("lifetime","2");
			pSnow->KeyValue("rendercolor", "217 156 0");
			pSnow->KeyValue("model", STRING( GetModelName() ) );
			DispatchSpawn( pSnow );
		}
		VPhysicsInitStatic();
...
char st_szOldMap[cchMapNameMost];
...
void CChangeLevel::ChangeLevelNow( CBaseEntity *pActivator )
{
	...
	//CBaseEntity *pPlayer = (pActivator && pActivator->IsPlayer()) ? pActivator : NULL;
	...
	st_szOldMap[0] = '\0';
	Q_strncpy( st_szOldMap, STRING( gpGlobals->mapname ), sizeof( st_szOldMap ) );
	...
	//Респаун первого игрока на месте перехода
	/*if( m_vecFinalDest != vec3_origin ) {
		CBaseEntity *pSpot = CreateEntityByName("info_player_transation");
		if( pSpot ) {
			pSpot->SetAbsOrigin( m_vecFinalDest );
			//Кривая камера если игрок переносился с уклоном
			//pSpot->SetAbsAngles( m_vecFinalAngle );
			DispatchSpawn( pSpot );
			variant_t emptyVariant;
			pSpot->AcceptInput( "InsideTransition", this, this, emptyVariant, 0 );
		}
	}*/

	//Меняем пока так, если карты нет, сменится по g_pGameRules->EndMultiplayerGame()
	if( filesystem->FileExists( UTIL_VarArgs( "maps/%s.bsp", st_szNextMap ) ) ) {
...
{
	CPropVehicleDriveable *m_hVehicle = dynamic_cast< CPropVehicleDriveable * >( pOther );
	if( m_hVehicle && m_hVehicle->GetDriver() )
		pOther = m_hVehicle->GetDriver();
	...
	m_vecFinalDest = pPlayer->GetAbsOrigin() + Vector( 0, 0, 6 );
	m_vecFinalAngle = pPlayer->GetAbsAngles();
	SetThink( &CChangeLevel::TimerThink );
	SetNextThink( gpGlobals->curtime );
	SetTouch( NULL );
	//ChangeLevelNow( pOther );
}
ConVar mp_transition_players_percent( "mp_transition_players_percent", "50" );

extern const char *UTIL_TranslateMsg( const char *szLanguage, const char *msg );

void CChangeLevel::TimerThink( void )
{
	if ( mp_transition_players_percent.GetInt() > 0 )
	{
		...
		for( int i = 1; i <= gpGlobals->maxClients; i++)
		{
			...
			if ( !pPlayer || pPlayer->IsAfk() || ( pPlayer->GetTeamNumber() == TEAM_SPECTATOR )  )
				continue;

			totalPlayers++;
			
			CBaseEntity *pEntities[1024];
			int iNumEntities = UTIL_EntitiesInSphere( pEntities, 1024, pPlayer->EyePosition(), 128.0, 0 );
			for ( int i = 0; i < iNumEntities; i++ )
			{
				CBaseEntity *pEntity = pEntities[i];
				if ( pEntity && pEntity == this ) {
						m_hTouchingEntities.AddToTail( pPlayer );
						break;
				}
			}
		}
		...
		{
			CBasePlayer *pPlayer = ToBasePlayer( m_hTouchingEntities[i] );
			if( !pPlayer )
				continue;

			const char* msg = UTIL_TranslateMsg( engine->GetClientConVarValue( ENTINDEX( pPlayer ), "cl_language" ), "#HL2MP_TrrigerCountMsg" );
			ClientPrint( pPlayer, HUD_PRINTCENTER, UTIL_VarArgs( "%s %d/%d", msg, m_hTouchingEntities.Count(), int( floor( totalPlayers / 100.0 * mp_transition_players_percent.GetInt() + 0.5 ) ) ) );
		}

		int result = int( floor( 100.0 / totalPlayers * m_hTouchingEntities.Count() + 0.5 ) );
		if ( result < mp_transition_players_percent.GetInt() ) {
			SetNextThink( gpGlobals->curtime + 0.1 );
			//Msg("Transitions: Not enough players to trigger level change %d\n", result);
			return;
		}
	}
	ChangeLevelNow( NULL );
